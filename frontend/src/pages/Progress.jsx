...
import { Flame, Trophy, BookOpen, Target, BarChart3, TrendingUp, Calendar, Zap, Monitor } from 'lucide-react';
...
export default function Progress() {
  ...
  if (loading) return (
    <div className="animate-fade-up" aria-busy="true" aria-label="Loading analytics">
      ...
      <div className="glass" style={{ padding: 24, marginBottom: 24 }}>
        ...
        {[1,2,3,4,5].map(i => <div key={i} style={{ marginBottom: 20 }} aria-hidden="true"><Skeleton height={14} width="80%" style={{ marginBottom: 8 }} /><Skeleton height={12} borderRadius={6} /></div>)}
      </div>
    </div>
  );

  if (!progress) return <div style={{ textAlign: 'center', padding: 80, color: 'var(--text-muted)' }}>No progress data available. Continue your preparation modules!</div>;
  ...
  const mockCount = progress.mockStats?.count || 0;
  const avgMockScore = progress.mockStats?.avgScore || 0;
  const prepScore = progress.prepScore || 0;
  ...
  const { insights } = useMemo(() => generateInsights(progress), [progress]);
  const tip = insights.find(i => i.includes('🎯')) || insights[0];
  ...
  return (
    <div className="animate-fade-up">
      ...
      >
        {[
          { icon: Trophy, label: 'Solved Modules', value: `${solvedCount}/${total || 60}`, color: '#a78bfa', bg: 'rgba(124, 58, 237, 0.1)' },
          { icon: Flame, label: 'Current Streak', value: `${streak} days`, color: '#f87171', bg: 'rgba(239, 68, 68, 0.1)' },
          { icon: Monitor, label: 'Mock Sessions', value: mockCount, color: '#38bdf8', bg: 'rgba(56, 189, 248, 0.1)' },
          { icon: TrendingUp, label: 'Preparation Score', value: prepScore, color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)' },
        ].map(({ icon: Icon, label, value, color, bg }, i) => (
          <motion.div key={i} variants={itemVariants} className="glass" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '20px', borderBottom: `2px solid ${color}33` }} role="status" aria-label={`${label}: ${value}`}>
            <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: color }}>
              <Icon size={24} />
            </div>
            <div>
      ...
      >
        ...
        <button className="btn btn-primary" style={{ padding: '14px 28px', borderRadius: '12px' }}>Strategic Protocol</button>
      </motion.div>
    </div>
  );
}
